...
class CRF(object):
    ...
    def get_word_features(self, sent, i):
        ...
        """
        # the i-th token
        word_feats = {'token': sent.tokens[i]}
        # TODO for question 1
        # the i-th tag
        # 
        # TODO for question 2
        ...
        word_feats['firstletterofword'] = sent.tokens[i][0]

        word_feats['lastletterofword'] = sent.tokens[i][-1]
        
        word_feats['firsttwoletterofword'] = sent.tokens[i][:2]

        word_feats['lasttwoletterofword'] = sent.tokens[i][-2:]

        previous_word= ""
        if i > 0:
            previous_word = sent.tokens[i-1]
        word_feats['previous'] = previous_word

        next_word= ""
        if i != len(sent.tokens) -1:
            next_word = sent.tokens[i+1]
        word_feats['next'] = next_word

        if sent.tokens[i].isdigit():
            word_feats['isdigit'] = True
        else:
            word_feats['isdigit'] = False

        if sent.tokens[i].isalnum()==False:
            word_feats['specialchar'] = True
        else:
            word_feats['specialchar'] = False

        word_feats['firstthreeletterofword'] = sent.tokens[i][:3]

        word_feats['lastthreeletterofword'] = sent.tokens[i][-3:]
